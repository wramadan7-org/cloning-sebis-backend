...
module.exports = {
	viewStudy: async(req, res) => {
		...
		try{
			...
			var getTypeCourse = await Crs.find().sort({name: 'asc'});
			var getGrade = await Grade.find();
			
			if(getStudy.length < 1 || getStudy == undefined) { 
				getStudy = '';
			}
			...
			new renderEJS(valuesData, req,res, 'admin/index.ejs', 'content-main/main-study.ejs', '', 'content-header/study-head.ejs');
		} catch(error){
	...
	addStudy: async (req, res) => {
		...
		try{
			...
			var {dataGrade, dataCourse} = req.body;
			...
			req.flash('postMessage', 'Study has been added!');
			
			return res.redirect('/mystudy');
		} catch(error){
			res.status(500).send({message: error.message});
		};
	},
	
	disableStudy: async(req, res) => {
		var regId = req.cookies.regId;
		...
		try{
			...
			if(std.identitiesId == getIdentities._id){
				var stats = std.disable;
				if(stats == "enable")
					stats = "disable";
				else
					stats = "enable";
				...
				req.flash('postStatus', 'success');
				req.flash('postMessage', 'Study has been ' + ucfirst(stats) + ' !');
			} else {
				req.flash('postStatus', 'error');
				req.flash('postMessage', 'Bad Request !');
			}
			return res.redirect('/mystudy');
		} catch(error){
			req.flash('postStatus', 'error');
	...
	deleteStudy: async(req, res) => {
		...
		try{
			...
			req.flash('postMessage', 'Mentor study has been deleted !');
			return res.redirect('/mystudy');
		} catch(error){
			req.flash('postStatus', 'error');
			req.flash('postMessage', error.message);
